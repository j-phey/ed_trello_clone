...
from marshmallow.validate import Length # Used in password Length
...
## User model definition area, below Card model
...
#schema definition area, below User Schema
...
def auth_register():
    ...
    # set the admin attribute to false by default when created
    user.admin = False
...
# Allow user to be signed in to take actions securely
...
def auth_login():
    ...
    if not user or not bcrypt.check_password_hash(user.password, user_fields["password"]): # Check the hashed pw
        return abort(401, description="Incorrect username or password")
    
    # Create the access token after making sure the login credentials were right:
    #create a variable that sets an expiry date
    expiry = timedelta(days=1)
    #create the access token
    access_token = create_access_token(identity=str(user.id), expires_delta=expiry)
